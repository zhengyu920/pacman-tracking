...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if(noisyDistance == None):
            ...
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition) # dist between 'legal pos p' and 'pacman'
                if emissionModel[trueDistance] > 0:
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            p = self.legalPositions[j]
            j = (j+1) % len(self.legalPositions)
    ...
    def observe(self, observation, gameState):
        ...
        if(noisyDistance == None):
            ...
            #print "observe (weights): \n", weights
            if(sum(weights) == 0):
                self.initializeUniformly(gameState)
                weights = [1 for i in range(N)]
            belief = util.Counter()
            for i in range(N):
                belief[self.particles[i]] = belief[self.particles[i]] + weights[i]
            #print "observe (belief): \n", belief
            newParticle = []
            for i in range(N):
                newParticle.append(util.sample(belief))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        permutations = itertools.product(self.legalPositions, self.legalPositions)
    ...
    def observeState(self, gameState):
        ...
        for g in range(self.numGhosts):
            if(noisyDistances[g] == None):
                for i in range(self.numParticles):
                    l = list(self.particles[i])
                    l[g] = self.getJailPosition(g)
                    self.particles[i] = tuple(l)
            else:
                #print "num particles: ", self.numParticles
                #print "list len: ", len(self.particles)
                #print "item len: ", len(self.particles[0])
                for i in range(self.numParticles):
                    #print "i : ", i
                    #print "g : ", g
                    distance = util.manhattanDistance(self.particles[i][g], pacmanPosition)
                    weights[i] = weights[i] * emissionModels[g][distance]
        #print "observe (weights): \n", weights
        if(sum(weights) == 0):
            self.initializeParticles()
            weights = [1 for i in range(self.numParticles)]
